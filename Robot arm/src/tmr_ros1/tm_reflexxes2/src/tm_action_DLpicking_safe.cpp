...
#define GRIPPER_LENGTH 0.235 //OBJECT1 and OBJECT2: 0.235  //OBJECT3, OBJECT4, OBJECT5: 0.205
                            ...
                            double SynTime)
{
    ...
    *IP = InputState;
    ...
    fprintf(f_obstacle, "===ReflexxesPositionRun===\n");    
    ...
    while (ResultValue != ReflexxesAPI::RML_FINAL_STATE_REACHED && ros::ok())
    {
        ...
        obs_distance = sqrt(pow((g_obstacle_position[0] - T[3]),2)+pow((g_obstacle_position[1] - T[7]),2)+pow((g_obstacle_position[2] - T[11]),2));
        fprintf(f_obstacle, "dist: %10.4lf, X: %10.4lf, Y: %10.4lf, Z: %10.4lf \n", obs_distance, g_obstacle_position[0] - T[3], g_obstacle_position[1] - T[7], g_obstacle_position[2] - T[11]);
    ...
    ROS_INFO("========== Final state ReflexxesPositionRun =============");
                                ...
                                double SynTime)
{
    ...
    *IP = InputState;
    ...
    gettimeofday(&tm3, NULL);
    fprintf(f_traj, "===ReflexxesPositionSafetyRun===\n");
    fprintf(f_obstacle, "===ReflexxesPositionSafetyRun===\n");
    ...
    while (ResultValue != ReflexxesAPI::RML_FINAL_STATE_REACHED)
    {
        ...
        tm_jacobian::Forward_Kinematics_gripper(q,T,GRIPPER_LENGTH);
        ROS_INFO("%10.4lf %10.4lf %10.4lf", T[3], T[7], T[11]);
        fprintf(f_traj, "gripper_XYZ: %10.4lf %10.4lf %10.4lf \n", T[3], T[7], T[11]);
        ...
        g_distance = sqrt(pow((g_obstacle_position[0] - T[3]),2)+pow((g_obstacle_position[1] - T[7]),2)+pow((g_obstacle_position[2] - T[11]),2));
        fprintf(f_obstacle, "dist: %10.4lf, X: %10.4lf, Y: %10.4lf, Z: %10.4lf \n", g_distance, g_obstacle_position[0] - T[3], g_obstacle_position[1] - T[7], g_obstacle_position[2] - T[11]);
        ...
        while (g_distance < DANGEROUS_ZONE)
        {
            ...
            if(OnlineRepulsiveForceGeneration(PotentialField_qd,T, q,RepulsiveVelocity))
            {
                ...
                if(CheckVelocityLimit(PotentialField_qd,ScalingFactor))
                {
                    ...
                    tm_jacobian::Forward_Kinematics_gripper(q,T,GRIPPER_LENGTH);
                    ROS_INFO("gripper_XYZ: %10.4lf %10.4lf %10.4lf Attractive: %10.4lf %10.4lf %10.4lf Repulsive: %10.4lf %10.4lf %10.4lf", T[3], T[7], T[11],TargetVelocity[0],TargetVelocity[1],TargetVelocity[2],RepulsiveVelocity[0],RepulsiveVelocity[1],RepulsiveVelocity[2] );
                    fprintf(f_traj, "gripper_XYZ: %10.4lf %10.4lf %10.4lf Attractive: %10.4lf %10.4lf %10.4lf Repulsive: %10.4lf %10.4lf %10.4lf \n", T[3], T[7], T[11],TargetVelocity[0],TargetVelocity[1],TargetVelocity[2],RepulsiveVelocity[0],RepulsiveVelocity[1],RepulsiveVelocity[2]);
                    
                    //ROS_INFO("RepulsiveForce: %10.4lf %10.4lf %10.4lf", T[3], T[7], T[11]);
                }
                else
                {
    ...
    if(pass == REACHPOINT)
    {
        ...
        fprintf(f_traj, "=============== Final state ReflexxesPositionSafetyRun =========================\n");
        fprintf(f_traj, "XYZ_pos: %10.4lf %10.4lf %10.4lf \n", T[3], T[7], T[11]);
...
bool BinPicking_Demo(TmDriver& TM5)
{
    ...
    //std::vector<double> ImageCpature = {0.0658,    -0.6272,     1.9238,     0.2836,     1.5775,     0.0598};
    /* 2018.07.22 updated by YuHsien
     * START POSITION without Container (lower) 
     * ImageCpature {0.0658,    -0.6272,     1.9238,     0.2836,     1.5775,     0.0598};
     * ImageCpature position (XYZ) = [   0.2192082 -0.1072567  0.3804391](m)
     * ImageCpature rotation (XYZ) = [  -3.131790885 -0.00615926693  1.576672](rad)
     ...
     * START POSITION without Container (lower) 
     * ImageCpature {0.0691,    -0.6283,     1.9241,     0.2848,     1.5714,     0.0632};
     * ImageCpature position (XYZ) = [    0.2192082 -0.1072567 0.3804391](m)
     * ImageCpature rotation (XYZ) = [  -3.131790885 0 1.576672](rad)
     * ImageCpature rotation (XYZ) = [-179.4384 0 90.3367](deg)
     */

    TargetVelocity = {0, 0, 0, 0, 0, 0};
    /*if(!ReflexxesPositionRun(TM5, *IP_position, ImageCpature, TargetVelocity, SynchronousTime))
    {
        ROS_WARN("Smooth stop activate...");
        return 0;
    }
        ...
        if(g_objposition[0] == 1.0 && position_fill && rotation_fill)
        {
            ...
            if(robot_motion_states.data == 1)
            {
                ...
                if(ReflexxesPositionSafetyRun(TM5, *IP_position, TargetPosition, TargetVelocity, SynchronousTime) == SMOOTHSTOP)
                    break;
                cout << "=== In tm_action_DLpicking_safe ===" << endl;
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "tm_action_DLpicking_safe");
