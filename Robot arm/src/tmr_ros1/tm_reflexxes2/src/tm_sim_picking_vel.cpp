...
#define REPEATABILITY 0.005//0.01
...
int main(int argc, char **argv)
{
    ...
    std::vector<double> TargetPosition(6),   TargetCartesain(6),      TargetVelocity(6);
    std::vector<double> CurrentPosition(6),  qd(6), Attractive_qd(6), Repulsive_qd(6);
    std::vector<double> RepulsiveVelocity(6),PotentialVelocity(6);
    ...
    while(ros::ok())
    {
        if(g_objposition[0] == 1.0 && position_fill && rotation_fill)
        {
            for (int i = 0; i < NUMBER_OF_DOFS; ++i)
            {
                ...
                Target_q[i] = g_objposition[i+1];
            }

            if(robot_motion_states.data == 1) // safety
            {
                for (int i = 0; i < NUMBER_OF_DOFS; ++i)
                {
                    IP_velocity->CurrentPositionVector->VecData[i]     = IP_position->CurrentPositionVector->VecData[i];
                ...
                while(robot_motion_states.data == 1)
                {
                    for (int i = 0; i < NUMBER_OF_DOFS; ++i)
                        q[i] = IP_velocity->CurrentPositionVector->VecData[i];

                    tm_jacobian::Forward_Kinematics_gripper(Target_q,T,GRIPPER_LENGTH);
                    TargetCartesain={ T[3], T[7], T[11], -180*DEG2RAD, 0, 90*DEG2RAD};

                    tm_jacobian::Forward_Kinematics_gripper(q,T,GRIPPER_LENGTH);
                    if(OnlineAttractiveForceGeneration(Attractive_qd, T, q, TargetCartesain, TargetVelocity))
                        pass = ReflexxesVelocityRun_sim(*IP_velocity, Attractive_qd, TargetCartesain, TargetVelocity ,0.2);
                    else
                        break;
            ...
            else if(robot_motion_states.data == 4)//safety
            {
                for (int i = 0; i < NUMBER_OF_DOFS; ++i)
                {
                    IP_velocity->CurrentPositionVector->VecData[i]     = IP_position->CurrentPositionVector->VecData[i];
                ...
                while(robot_motion_states.data == 4)
                {
                    for (int i = 0; i < NUMBER_OF_DOFS; ++i)
                        q[i] = IP_velocity->CurrentPositionVector->VecData[i];
        ...
        else if(g_objposition[0] == 2.0)//safety
        {
            SynchronousTime = 3;
            if(!ReflexxesPositionRun_sim(*IP_position, ImageCpature, TargetVelocity, SynchronousTime))
                break;
