...
bool BinPicking_Demo()
{
    ...
    //0523 箱子image_pose
    std::vector<double> ImageCapture1    = {1.0591,    0.1066,    1.6258,    -0.1542,    1.5723,    -0.505};
    //0524 桌角image_pose
        ...
        if(g_objposition[0] == 1.0 && position_fill && rotation_fill)
        {
            ...
            else if(robot_motion_states.data == 4)
            {
                ...
                if(!table_to_box)
                {
                    ...
                    if(ReflexxesPositionRun(*IP_position, TargetPosition, TargetVelocity, SynchronousTime) == SMOOTHSTOP)
                        break;
                ...
                else
                {
                    ...
                    //箱子內彷初始排法
                    if(finalpose_flag == 1)
                        final_position = {0.9945,    0.7611,    0.4229,    0.3955,    1.5723,    -0.5669};


                    else if(finalpose_flag == 2)
                        final_position = {1.0334,    0.429,    0.9594,    0.1907,    1.5728,    -0.5289};


                    else if(finalpose_flag == 3)
                        final_position = {1.1081,    0.7451,    0.4516,    0.3822,    1.5728,    -0.4536};


                    else if(finalpose_flag == 4)
                        final_position = {1.174,    0.4032,    0.9925,    0.1781,    1.5692,    -0.3844};


                    else if(finalpose_flag == 5)
                        final_position = {1.0365,    0.6409,    0.6301,    0.3078,    1.5729,    -0.5255};
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        {
            std::vector<double> vec1 = {1.0591,    0.1066,    1.6258,    -0.1542,    1.5723,    -0.505};
