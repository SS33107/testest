...
bool BinPicking_Demo(TmDriver& TM5)
{
    ...
    while(ros::ok())
    {
        ...
        {
            SynchronousTime = 2;
            if(ReflexxesPositionRun(TM5, *IP_position, ImageCpature, TargetVelocity, SynchronousTime) == SMOOTHSTOP)
                break;
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        {
            double blend = 0;
            std::vector<double> vec = {1.0076,    0.2228,    0.9947,    0.3476,    1.5761,    -0.5633};
