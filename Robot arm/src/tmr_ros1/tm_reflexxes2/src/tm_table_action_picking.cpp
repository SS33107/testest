...
#define REPEATABILITY 0.0001//0.00005
...
std::vector<int> Grasp_ObjectList;
int left_object_number=0;
using namespace std;
...
bool BinPicking_Demo(TmDriver& TM5)
{
     ...
     */
    //std::vector<double> ImageCapture = {0.0377,    -0.4377,    1.8617,    0.1746,    1.5799,    0.0361};
    std::vector<double> ImageCapture = {0.1959,    -0.5078,    1.8021,    0.2807,    1.5647,    0.2036};
    std::vector<double> box_above = {1.2982,    0.024,    1.5084,    0.0228,    1.5365,    0.2138};
...
*/
    ...
    while(ros::ok())
    {
        if(g_objposition[0] == 1.0 && position_fill && rotation_fill)
        {
            ...
            {
                //table_prepare
                SynchronousTime = 1.3;//1.3;//3; //2;
                robot_motion_states.data = 4;
                left_object_number = left_object_number - 1;
                if(!ReflexxesPositionRun(TM5, *IP_position, TargetPosition, TargetVelocity, SynchronousTime))
                    break;
                if(left_object_number == 0)
                {   
                    SynchronousTime = 1;
                    if(!ReflexxesPositionRun(TM5, *IP_position, ImageCapture, TargetVelocity, SynchronousTime))
                        break;
            ...
            else if(robot_motion_states.data == 4)
            {
                ...
                if(!ReflexxesPositionRun(TM5, *IP_position, box_above, TargetVelocity, SynchronousTime))
                    break;  
                TM5.setDigitalOutputEE(0,true);//gripper open
                RobotState.publish(robot_motion_states);
                ROS_WARN("Robot State = %d Complete",robot_motion_states.data);
                robot_motion_states.data = 1;
                // ROS_INFO("Enter to CONTINUE");
...
void left_Callback(const my_msg_pkg::pose_estimation_data::ConstPtr& pose_data)
{

    Grasp_ObjectList.clear();

    ROS_WARN("Receiving data !!!");
    for(int i = 0; i < pose_data->Grasp_ObjectList_from_pose_estimation.size(); i++)
    {
        Grasp_ObjectList.push_back(pose_data->Grasp_ObjectList_from_pose_estimation[i]);
    }
    left_object_number = Grasp_ObjectList.size();
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber left_sub = node_handle.subscribe("/pose_estimation", 1, left_Callback);
    spinner.start();
    ...
    while (ros::ok())
    {
        ...
        else if(strncmp(cstr, "image", 5) == 0)
        {
            double blend = 0;
            std::vector<double> vec1 = {0.1959,    -0.5078,    1.8021,    0.2807,    1.5647,    0.2036};
            TmRobot.setMoveJabs(vec1, blend);
            print_info("image_pose");
        }
