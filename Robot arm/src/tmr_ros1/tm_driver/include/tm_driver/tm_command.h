...
/*	Start joint velocity mode.
	VelMode 			: Velocity mode, Joint and Tool
	timeout_zero_vel 	: Used in Tool velocity mode, If you do not renew the velocity data after this time in secoonds, set the speed down to zero.
	timeout_stop 		: Used in Tool velocity mode, If you do not renew the velocity data after this time in secoonds, stop the End-Effector velocity mode. */
	static std::string set_vel_mode_start(VelMode mode, double timeout_zero_vel, double timeout_stop);

/*	Stop joint velocity mode */
	static std::string set_vel_mode_stop() { return "StopContinueVmode()"; }

/*	Set joint velocity.
	VelMode : Velocity mode, Joint and Tool
	vel : Velocity data
	precision : Set floating number presision */
